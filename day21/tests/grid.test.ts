...
describe('test getPointAtLocationInfiniteGrid function', () => {
    it('on a 3x3 grid should be able to get data for point -1, -1', async () => {
        let infiniteGridPoint = await grid.getPointAtLocationInfiniteGrid(-1, -1)
        expect(infiniteGridPoint.point.value).toBe("9")
        expect(infiniteGridPoint.point.row).toBe(2)
        expect(infiniteGridPoint.point.column).toBe(2)
        expect(infiniteGridPoint.gridId.x).toBe(-1)
        expect(infiniteGridPoint.gridId.y).toBe(-1)
    })
})
